<?php
    ...
    echo $imageTemp;
    if(!empty($nomPhoto)){
        $requete="update Etudiant set nom=?,prenom=?,civilite=?,idFiliere=?,photo=? where idEtudiant=?";
        $params=array($nom,$prenom,$civilite,$idFiliere,$nomPhoto,$idS);
    }else{
        $requete="update Etudiant set nom=?,prenom=?,civilite=?,idFiliere=? where idEtudiant=?";
